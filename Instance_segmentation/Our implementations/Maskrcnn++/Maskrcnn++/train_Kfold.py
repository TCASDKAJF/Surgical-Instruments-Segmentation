...
# The modified main function to use Stratified K-Fold cross-validation with loss calculation at the end of each fold.

def main_complete(args):
    ...
    full_dataset = CocoDetection(data_root, "train", mode='train')
    # Assuming that the dataset provides a `get_labels` method to get all the labels
    labels = full_dataset.get_labels()
    
    # Stratified K-Fold
    skf = StratifiedKFold(n_splits=5, random_state=42, shuffle=True)

    fold_losses = []  # To store the loss for each fold
    ...
    for fold, (train_index, val_index) in enumerate(skf.split(np.zeros(len(labels)), labels)):
        ...
        # Learning rate scheduler for each fold
        lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,
                                                            milestones=args.lr_steps,
                                                            gamma=args.lr_gamma)
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            mean_loss, lr = utils.train_one_epoch(model, optimizer, train_data_loader,
                                                  device, epoch, print_freq=50,
                                                  warmup=True)
            fold_losses.append(mean_loss.item())
            ...
            # Write detection results into txt with fold information
            with open(det_results_file, "a") as f:
                result_info = [f"{i:.4f}" for i in det_info + [mean_loss.item()]] + [f"{lr:.6f}"]
                txt = f"fold:{fold + 1} epoch:{epoch} {'  '.join(result_info)}"
                f.write(txt + "\n")

            # Write segmentation results into txt with fold information
            with open(seg_results_file, "a") as f:
                result_info = [f"{i:.4f}" for i in seg_info + [mean_loss.item()]] + [f"{lr:.6f}"]
                txt = f"fold:{fold + 1} epoch:{epoch} {'  '.join(result_info)}"
                f.write(txt + "\n")
            ...
            # Save weights with epoch and fold information
            save_files = {
                'model': model.state_dict(),
                'optimizer': optimizer.state_dict(),
                'lr_scheduler': lr_scheduler.state_dict(),
            ...
            if args.amp:
                save_files["scaler"] = scaler.state_dict()
            torch.save(save_files, f"./save_weights/fold_{fold + 1}_model_{epoch}.pth")


        # Compute the average loss for this fold
        avg_fold_loss = sum(fold_losses) / len(fold_losses)
        print(f"Average loss for fold {fold + 1}: {avg_fold_loss:.4f}")

    # Compute the average loss across all folds
    avg_loss = sum(fold_losses) / len(fold_losses)
    print(f"Average loss across all folds: {avg_loss:.4f}")
