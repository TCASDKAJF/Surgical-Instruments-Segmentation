...
import transforms
# from backbone import resnet50_fpn_models
...
def main(parser_data):
    ...
    # test_dataset = CocoDetection(data_root, "test", data_transform["test"])
    ...
    # det_metric = EvalCOCOMetric(test_dataset.coco, "bbox", "det_results.json")
    # seg_metric = EvalCOCOMetric(test_dataset.coco, "segm", "seg_results.json")
    ...
    with torch.no_grad():
        ...
        # for image, targets in tqdm(test_dataset_loader, desc="test..."):
            ...
            # 增加mIoU的计算
            gt_masks = [item['masks'].numpy() for item in targets]
            pred_masks = [item['masks'].numpy() for item in outputs]
            miou_list.append(compute_mIoU(gt_masks, pred_masks, len(category_index)))
    ...
    save_info(det_metric.coco_evaluator, category_index, "det_record_mAP.txt", additional_info=f"mIoU: {average_miou:.3f}")
    save_info(seg_metric.coco_evaluator, category_index, "seg_record_mAP.txt", additional_info=f"mIoU: {average_miou:.3f}")
...
if __name__ == "__main__":
    ...
    # 数据集的根目录
    parser.add_argument('--data-path', default='./data/coco', help='dataset root')

    # 训练好的权重文件
    parser.add_argument('--weights-path', default='/root/autodl-tmp/new_swin/Detection_Baseline_swin/save_weights/CA_CBAM/model_27.pth', type=str, help='training weights')
