...
class SwinBackbone(nn.Module):
    def __init__(self, backbone: str, pretrain_backbone: str):
        super().__init__()
        ...
        # Initialize weights using pretrain_backbone
        self.body.init_weights(pretrain_backbone)
...
def build_backbone(args):
    if "resnet50" in args.backbone:
        ...
        resnet_backbone = Backbone(args.backbone, train_backbone, return_interm_layers, args.dilation)
        ...
        swin_backbone = SwinBackbone(args.backbone, args)
        out_channels = 256
        if "swin_tiny" in args.backbone:
            in_channels_list = [96, 192, 384, 768]
        elif "swin_small" in args.backbone:
            in_channels_list = [96, 192, 384, 768]
        else:
...
def swin_fpn_backbone(backbone_type: str, pretrain_backbone: str) -> nn.Module:
    """
    构建一个基于Swin Transformer的FPN背景模型。

    参数:
        backbone_type (str): Swin Transformer的类型，如 "swin_tiny", "swin_small" 等。
        pretrain_backbone (str): Swin Transformer预训练模型的路径。

    返回:
        nn.Module: 带有FPN的Swin Transformer背景模型。
    """
    
    swin_backbone = SwinBackbone(backbone_type, pretrain_backbone)
    out_channels = 256
    if "swin_tiny" == backbone_type:
        in_channels_list = [96, 192, 384, 768]
    elif "swin_small" == backbone_type:
        in_channels_list = [96, 192, 384, 768]
    else:
        in_channels_list = [192, 384, 768, 1536]
    backbone = BackboneWithFPN(swin_backbone, in_channels_list, out_channels)
